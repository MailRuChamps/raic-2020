...
- `world_pos`: `Option<Vec2Float>` - Позиция в мировых координатах (если отсутствует, используются координаты (0, 0) экрана)
- `screen_offset`: `Vec2Float` - Дополнительное смещение в экранных координатах
- `color`: `Color` - Цвет
...
- `initial_cost`: `int32` - Стоимость постройки первой сущности данного типа. Каждая следующая будет стоить на 1 больше
...
- `center`: `Vec2Float` - Точка на которую смотрит камера
...
- `mouse_pos_window`: `Vec2Float` - Положение курсора в оконных координатах
- `mouse_pos_world`: `Vec2Float` - Положение курсора в мировых координатах
